// /abalat/events/[eventId]/attendance/components/AttendanceGrid.tsx
'use client'

import { useState, useEffect } from 'react';
import { useRouter } from 'next/navigation';
import { Check, CheckCircle2, Loader2, X, Clock, Users, Shield } from 'lucide-react';
...
interface AttendanceGridProps {
  ...
  memberPermissions: Map<string, MemberPermission>;
}
...
export default function AttendanceGrid({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/abalat/events/${eventId}/attendance`, {
        method: 'POST',
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {memberPermissions.get(member.id)?.hasPermission && (
                          <div
                            ...
                            title={`Permission: ${memberPermissions.get(member.id)?.permissionType}${memberPermissions.get(member.id)?.reason ? ` - ${memberPermissions.get(member.id)?.reason}` : ''}`}
                          >
                            <Shield size={10} />
                            <span>ፈቃድ</span>
                          </div>
