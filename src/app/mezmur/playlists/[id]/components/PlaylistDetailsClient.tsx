...
import { Play, Pause, Trash2, FileText, ArrowLeft, Music, Download, Loader2, X as CloseIcon, AlignLeft, AlignRight, Languages, Upload, Plus } from "lucide-react";
...
export default function PlaylistDetailsClient({ playlist, allCategories, adminId }: PlaylistDetailsClientProps) {
  ...
  const handleAddExisting = async (musicFileIds: string[]) => {
    const res = await fetch(`/api/mezmur/playlists/${playlist.id}/songs`, {
      method: "POST",
