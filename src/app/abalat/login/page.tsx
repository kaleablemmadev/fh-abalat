...
export default function AbalatLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/abalat/auth/login', {
        method: 'POST',
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/abalat/auth/change-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId,
      ...
      <div className="w-full max-w-md space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="superadmin@abalat.com"
                className="w-full pl-10 pr-4 py-2 rounded border text-sm"
                style={{
        ...
        <div className="text-center pt-4 border-t" style={{ borderColor: 'hsl(var(--border))' }}>
          <p className="text-xs" style={{ color: 'hsl(var(--muted-foreground))' }}>
            New admin? <a href="/abalat/admin-registration" className="font-semibold underline" style={{ color: 'hsl(var(--primary))' }}>Register here</a>
          </p>
        </div>
      </div>
    </div>
