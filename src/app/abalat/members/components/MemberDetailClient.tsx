// /abalat/members/components/MemberDetailClient.tsx
"use client";
...
export default function MemberDetailClient({ memberId }: { memberId: string }) {
  ...
  useEffect(() => {
    ...
    async function loadPermissions() {
      try {
        const res = await fetch(`/api/abalat/members/${memberId}/permissions`);
        if (res.ok) {
  ...
  useEffect(() => {
    async function loadPermissionTypes() {
      try {
        const res = await fetch('/api/abalat/permission-types');
        if (res.ok) {
          const data = await res.json();
  ...
  const handleAddPermission = async () => {
    ...
    try {
      const monthName = ethMonthNames[parseInt(startMonth)];
      const formattedDate = `${monthName} ${startDay}, ${startYear}`;
      ...
      const res = await fetch(`/api/abalat/members/${memberId}/permissions`, {
        ...
        body: JSON.stringify({
          ...
          ethiopianStartDate: formattedDate,
        }),
      });
  ...
  const handleDeletePermission = async (permissionId: string) => {
    ...
    try {
      const res = await fetch(`/api/abalat/members/${memberId}/permissions/${permissionId}`, {
        method: 'DELETE',
      });
  ...
  const handleOpenAddPermission = () => {
    ...
    setStartDay(today.day.toString());
    setStartYear(today.year.toString());
        ...
        {isLoading ? (
          ...
          <div>
            ...
            >
              ...
              <Link
                href={`/abalat/members/${memberId}/edit`}
