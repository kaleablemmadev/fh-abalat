...
interface CoursePermissionListProps {
  initialPermissions: any[];
  permissionTypes: any[];
  students: any[];
}

export default function CoursePermissionList({
  initialPermissions,
  permissionTypes,
  students
}: CoursePermissionListProps) {
  ...
  const [newPermission, setNewYear] = useState({
    memberId: "",
    permissionTypeId: "",
    reason: "",
    ethiopianStartDate: "",
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch("/api/course/permissions", {
        method: "POST",
  ...
  const handleUpdateStatus = async (id: string, status: "APPROVED" | "REJECTED") => {
    try {
      const res = await fetch(`/api/course/permissions/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-[hsl(217,70%,32%)] text-white rounded-lg font-semibold text-sm whitespace-nowrap"
          >
            <Plus size={16} />
            Add Permission
          </button>
        ...
        <form onSubmit={handleCreate} className="p-6 bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-xl shadow-lg space-y-4 animate-slide-in">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-xs font-bold uppercase opacity-50">Student</label>
              <select
                ...
                onChange={e => setNewYear({ ...newPermission, memberId: e.target.value })}
                required
              >
            ...
            <div className="space-y-1">
              ...
              <select
                ...
                onChange={e => setNewYear({ ...newPermission, permissionTypeId: e.target.value })}
                required
              >
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                onChange={e => setNewYear({ ...newPermission, ethiopianStartDate: e.target.value })}
                required
              />
            </div>
            <div className="space-y-1">
              ...
              <input
                ...
                onChange={e => setNewYear({ ...newPermission, ethiopianEndDate: e.target.value })}
                required
              />
            </div>
            <div className="space-y-1 md:col-span-2">
              ...
              <textarea
                ...
                onChange={e => setNewYear({ ...newPermission, reason: e.target.value })}
              />
            </div>
          </div>
          <div className="flex justify-end gap-3 pt-4 border-t border-[hsl(var(--border))]">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="flex items-center gap-2 px-6 py-2 bg-[hsl(217,70%,32%)] text-white rounded-lg font-semibold text-sm"
            >
        ...
        {filteredPermissions.map((p) => (
          <div key={p.id} className="p-4 bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              {p.status !== "PENDING" && (
                <div className="text-[10px] uppercase font-bold tracking-widest opacity-50 px-3 py-1 bg-zinc-100 rounded-full">
