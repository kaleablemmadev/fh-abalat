// /mezmur/superadmin-settings/page.tsx
'use client';
...
export default function MezmurSuperadminSettingsPage() {
  ...
  const fetchAdmins = async () => {
    try {
      const res = await fetch('/api/mezmur/superadmin/admins');
      if (res.ok) {
  ...
  const handleEmailPasswordChange = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/mezmur/superadmin/update', {
        method: 'POST',
  ...
  const handleTransferOwnership = async (e: React.FormEvent) => {
    ...
    if (!confirm('Are you sure you want to transfer Mezmur superadmin ownership? This action cannot be undone.')) {
      return;
    }
    ...
    try {
      const response = await fetch('/api/mezmur/superadmin/transfer', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleDeleteAdmin = async (adminId: string, adminName: string) => {
    if (!confirm(`Are you sure you want to delete Mezmur admin "${adminName}"? They will be notified of this action.`)) {
      return;
    }
    ...
    try {
      const response = await fetch('/api/mezmur/superadmin/admins', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="space-y-4">
        ...
        <div className="rounded border p-6 space-y-6" style={{ background: 'hsl(var(--card))', borderColor: 'hsl(var(--border))' }}>
          <div>
            ...
            <form onSubmit={handleEmailPasswordChange} className="space-y-4">
              ...
              {success && (
                <div className="p-3 rounded text-sm" style={{ background: 'hsl(25 40% 12%)', color: 'hsl(25 60% 55%)' }}>
                  {success}
                </div>
              )}
        ...
        <div className="rounded border p-6 space-y-6" style={{ background: 'hsl(var(--card))', borderColor: 'hsl(var(--border))' }}>
          <div>
            <h2 className="text-lg font-semibold mb-4 flex items-center gap-2" style={{ color: 'hsl(var(--foreground))' }}>
              <AlertTriangle size={18} style={{ color: 'hsl(45 70% 45%)' }} />
              Transfer Superadmin Ownership
            </h2>
            <p className="text-sm mb-4" style={{ color: 'hsl(var(--muted-foreground))' }}>
              Transfer superadmin ownership to another admin. This action cannot be undone.
            </p>
            <form onSubmit={handleTransferOwnership} className="space-y-4">
              <div className="space-y-2">
                <label htmlFor="transferToAdmin" className="text-sm font-medium" style={{ color: 'hsl(var(--foreground))' }}>
                  Select Admin
                </label>
                <select
                  id="transferToAdmin"
                ...
                >
                  ...
                  {admins.map((admin) => (
                    <option key={admin.id} value={admin.id}>
                      {admin.fullName || 'Unknown'} ({admin.email || 'No email'})
                    </option>
                  ))}
                </select>
              </div>
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full py-2 px-4 rounded text-sm font-medium transition-colors duration-150 disabled:opacity-50"
                style={{
        ...
        <div className="rounded border p-6 space-y-6" style={{ background: 'hsl(var(--card))', borderColor: 'hsl(var(--border))' }}>
          <div>
            <h2 className="text-lg font-semibold mb-4" style={{ color: 'hsl(var(--foreground))' }}>
              Manage Mezmur Admins
            </h2>
            {admins.length === 0 ? (
              <p className="text-sm" style={{ color: 'hsl(var(--muted-foreground))' }}>
                No Mezmur admins found.
              </p>
            ) : (
                ...
                {admins.map((admin) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      disabled={isLoading}
                      className="p-2 rounded text-sm transition-colors duration-150 disabled:opacity-50"
                      style={{
        ...
        {/* Logout */}
        <div className="rounded border p-6" style={{ background: 'hsl(var(--card))', borderColor: 'hsl(var(--border))' }}>
          <button
