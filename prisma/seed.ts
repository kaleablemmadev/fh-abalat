...
async function main() {
  ...
  const superadmins = [
    { fullName: 'Abalat Superadmin', email: 'superadmin@abalat.com', mode: 'ABALAT', password: 'abalat' },
    { fullName: 'Course Superadmin', email: 'superadmin@course.com', mode: 'COURSE', password: 'course' },
    { fullName: 'Mezmur Superadmin', email: 'superadmin@mezmur.com', mode: 'MEZMUR', password: 'mezmur' },
  ];

  let mainAdminId: string | null = null;
  ...
  for (const sa of superadmins) {
    ...
    if (!existing) {
      ...
      console.log(`  ✅ Created superadmin: ${sa.fullName}`);
      if (!mainAdminId || sa.mode === 'ABALAT') mainAdminId = created.id;
    } else {
      console.log(`  ⏭️  Superadmin already exists: ${sa.fullName}`);
      if (!mainAdminId || sa.mode === 'ABALAT') mainAdminId = existing.id;
    }
  }

  const adminId = mainAdminId || "dummy-admin-id";
  ...
  if (existingMembers === 0) {
    ...
    for (const member of members) {
      const hash = await hashPassword('member123');
      const created = await prisma.user.create({
        data: {
          ...member,
          type: 'MEMBER',
      ...
      console.log(`  ✅ Created member: ${created.fullName} with code ${created.privateId}`);
    }
  } else {
  ...
  async function createEventIfNotExists(
    ...
    if (!existing) {
      const eventType = title.toLowerCase().includes('chore') ? 'CHORE' :
                        title.toLowerCase().includes('sunday') ? 'SUNDAY' : 'EVENT';
