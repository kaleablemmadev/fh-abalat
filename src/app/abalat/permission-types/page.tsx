// /abalat/permission-types/page.tsx
'use client';
...
export default function PermissionTypesPage() {
  ...
  const fetchTypes = async () => {
    ...
    setError(null);
    try {
      const response = await fetch('/api/abalat/permission-types');
      if (!response.ok) {
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeletingId(id);
    try {
      const response = await fetch(`/api/abalat/permission-types/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: 'hsl(var(--muted-foreground))' }}>
            የአባላት አቴንዳንስ ፈቃዶች መመዝገቢያና መከታተያ ድኅረ ገጽ
          </p>
        </div>
        <Link
            ...
            {types.map((type) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <Link
                    href={`/abalat/permission-types/${type.id}/edit`}
