      ...
      sections: [{
        ...
        children: [
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: `የአቴንዳንስ ዐይነት: ${attendanceType === 'ALL' ? 'ሁሉም (የሠርክና የእሑድ)' : attendanceType === 'chore'? 'የሠርክ ብቻ' : 'የእሑድ ጉባዔ ብቻ'}`,
                size: 18,
              }),
            ],
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: `የተዘጋጀው በ: ${new Date().toLocaleDateString()}`,
                size: 18,
              }),
            ],
      ...
      sections: [{
        ...
        children: [
          ...
          ...(eventDate ? [new Paragraph({
            children: [
              new TextRun({
                text: `ቀን: ${eventDate.toLocaleDateString()} | ሰዓት: ${eventDate.toLocaleTimeString()}`,
                size: 16,
              }),
            ],
            alignment: AlignmentType.CENTER,
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: `አጠቃላይ አባላት: ${totalMembers}`,
                size: 18,
                bold: true,
              }),
              new TextRun({
                text: ` | መስፈርት ያሟሉ: ${eligibleMembers.length}`,
                size: 18,
                bold: true,
                color: '2e7d32',
              }),
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: `የተመዘገበው በ: ${new Date().toLocaleString()}`,
                size: 14,
                color: '666666',
              }),
            ],
  ...
  static async generatePDF(options: DocumentOptions): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        try {
          const fontPaths = [
            path.join(/*turbopackIgnore: true*/ process.cwd(), 'src', 'assets', 'fonts', 'NotoSansEthiopic-VariableFont_wdth,wght.ttf'),
            path.join(/*turbopackIgnore: true*/ process.cwd(), 'src', 'assets', 'fonts', 'NotoSansEthiopic-Regular.ttf'),
          ];
        ...
        doc.setFontSize(10);
        doc.setFont('helvetica', 'bold');
        doc.text(`አጠቃላይ አባላት: ${totalMembers}`, 15, yPosition);
        doc.setTextColor(46, 125, 50);
        doc.text(`መስፈርት ያሟሉ: ${eligibleMembers.length}`, 85, yPosition);
        doc.setTextColor(0);
        yPosition += 5;
        ...
        doc.setFont('helvetica', 'normal');
        doc.text(`የተዘጋጀው በ: ${new Date().toLocaleString()}`, pageWidth - 15, finalY + 8, { align: 'right' });
