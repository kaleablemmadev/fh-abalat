// /abalat/events/[eventId]/eligibility/components/EligibilityReportClient.tsx
'use client';
...
export default function EligibilityReportClient({
  ...
  const fetchReport = async (ruleId: string) => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/abalat/events/${eventId}/eligibility?ruleId=${ruleId}`);
      if (!response.ok) {
  ...
  const applyRuleToEvent = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/abalat/events/${eventId}/eligibility-rule`, {
        method: 'PUT',
        headers: {
  ...
  const downloadReport = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/abalat/events/${eventId}/eligibility/download`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    <div className="space-y-6">
      ...
      <Breadcrumb
        items={[
          ...
          { label: eventTitle, href: `/abalat/events/${eventId}` },
          { label: 'መስፈርት' },
        ]}
      />
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[hsl(var(--border))]">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <Link
            href={`/abalat/events/${eventId}/attendance`}
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-end gap-3">
          ...
          >
            ...
            {isApplyingRule ? 'ትግበራ ላይ...' : 'ለበዓሉ መስፈርቱን ተግብር'}
          </button>
        </div>
      ...
      {isLoading ? (
        ...
        >
          ...
          <h2 className="text-lg font-semibold mb-2" style={{ color: 'hsl(var(--foreground))' }}>
            የአገልግሎት መስፈርት አልተመረጠም
          </h2>
          <p className="text-sm" style={{ color: 'hsl(var(--muted-foreground))' }}>
            መስፈርት የማሟላት ሪፖርቱን ለመመልከት ከላይ ካሉት የአገልግሎት መስፈርቶች ውስጥ ምረጥ
          </p>
          {allRules.length === 0 && (
            <Link
              ...
              {report.eligibilityRule.criteria.map((criterion, index) => (
                <li key={index} className="text-xs" style={{ color: 'hsl(var(--muted-foreground))' }}>
                  {criterion.isTotalAttendance ? (
                    <>አጠቃላይ: በትንሹ {criterion.minAttendances} አቴንንዳንሶች ባለፉት {criterion.lookbackMonths} ወራት</>
                  ) : (
                    <>{criterion.eventType.charAt(0).toUpperCase() + criterion.eventType.slice(1)}: 
                    በትንሹ {criterion.minAttendances} አቴንንዳንሶች ባለፉት {criterion.lookbackMonths} ወራት</>
                  )}
                </li>
              ))}
            </ul>
                        ...
                        >
                          ...
                          <div className="flex items-center gap-3 mt-1 text-[10px]" style={{ color: 'hsl(var(--muted-foreground))' }}>
                            {member.scores.requiredChore > 0 && (
                              <span>የሠርክ: {member.scores.choreScore}/{member.scores.requiredChore}</span>
                            )}
                            {member.scores.requiredSunday > 0 && (
                              <span>የእሑድ: {member.scores.sundayScore}/{member.scores.requiredSunday}</span>
                            )}
                            {member.scores.requiredTotal > 0 && (
                              <span>አጠቃላይ: {member.scores.totalScore}/{member.scores.requiredTotal}</span>
                            )}
                          </div>
