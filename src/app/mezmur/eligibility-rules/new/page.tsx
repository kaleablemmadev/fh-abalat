// /mezmur/eligibility-rules/new/page.tsx
'use client';
...
export default function NewMezmurEligibilityRulePage() {
  ...
  const [criteria, setCriteria] = useState<Criterion[]>([
    { id: '1', eventType: 'mezmur', minAttendances: 5, lookbackMonths: 2, isTotalAttendance: false },
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/mezmur/eligibility-rules', {
        method: 'POST',
    ...
    <div className="space-y-6 animate-fade-in max-w-2xl">
      ...
      <div>
        <h1 className="text-2xl font-bold tracking-tight" style={{ color: 'hsl(var(--foreground))' }}>
          የመዝሙር ማሟላት መስፈርት መዝግብ
        </h1>
        <p className="text-sm mt-0.5" style={{ color: 'hsl(var(--muted-foreground))' }}>
          በአንድ የመዝሙር በዓል አባላት እንዲያገለግት የሚወስን መስፈርት አስቀምጥ
        </p>
      </div>
