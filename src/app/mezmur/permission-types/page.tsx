// /mezmur/permission-types/page.tsx
'use client';
...
export default function MezmurPermissionTypesPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeletingId(id);
    try {
      const response = await fetch(`/api/mezmur/permission-types/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
  ...
  const getCategoryLabel = (category: string) => {
    return category === 'DURATION_BASED' ? 'በጊዜ ገደብ' : 'በቀን መሠረት';
  };

  const getCategoryDescription = (type: PermissionType) => {
    if (type.category === 'DURATION_BASED') {
      const parts = [];
      if (type.durationMonths) parts.push(`${type.durationMonths} ወራት`);
      if (type.durationYears) parts.push(`${type.durationYears} ዓመታት`);
      const appliesTo = [];
      if (type.appliesToChore) appliesTo.push('የሠርክ');
      if (type.appliesToSunday) appliesTo.push('የእሑድ');
      return `ለ${parts.join(', ')} (${appliesTo.join(' እና ')}) ፈቃድ ተሰጥቷል`;
    } else {
      const parts = [];
      if (type.specificDays.length > 0) {
        parts.push(`ቀናት: ${type.specificDays.join(', ')}`);
      }
      if (type.appliesToSundays) {
        parts.push('እሑዶች በሙሉ');
      }
      return parts.join(', ') || 'የተመዘገቡ ውስን ቀናት የሉም';
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: 'hsl(var(--muted-foreground))' }}>
            የዘማሪዎች አቴንዳንስ ፈቃዶች መመዝገቢያና መከታተያ ድኅረ ገጽ
          </p>
        </div>
        <Link
          href="/mezmur/permission-types/new"
            ...
            {types.map((type) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <Link
                    href={`/mezmur/permission-types/${type.id}/edit`}
