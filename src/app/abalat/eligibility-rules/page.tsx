// /abalat/eligibility-rules/page.tsx
'use client';
...
export default function EligibilityRulesPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeletingId(id);
    try {
      const response = await fetch(`/api/abalat/eligibility-rules/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
        ...
        {rules.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-12">
            ...
            <p className="text-lg font-medium" style={{ color: 'hsl(var(--foreground))' }}>
              ምንም የአገልግሎት መስፈርት አልተመዘገበም
            </p>
            <p className="text-sm mt-1" style={{ color: 'hsl(var(--muted-foreground))' }}>
              የመጀመሪያ የአገልግሎት መስፈርቱን በመመዝግብ ጀምር
            </p>
          </div>
        ) : (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1 flex-wrap">
                    {rule.criteria.map((c, index) => (
                      ...
                      >
                        {c.isTotalAttendance ? (
                          <>አጠቃላይ (የሠርክና የእሑድ): {c.minAttendances} አቴንዳንሶች በ{c.lookbackMonths} ወራት</>
                        ) : (
